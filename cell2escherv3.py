    ...
    for reaction in reactions:
        ...
        if len(reactants) > 0:
            ...
            for reactant in reactants:
                is_primary = reactant.get('@species') is not None
                metabolite_id = reactant.get('@species') if is_primary else reactant.get('@reactant')
                metabolites.append({
                    "bigg_id": get_metabolite_name(metabolite_id, nodes, is_primary),
                    "coefficient": -1
                })
        ...
        if len(products) > 0:
            ...
            for product in products:
                is_primary = product.get('@species') is not None
                metabolite_id = product.get('@species') if is_primary else product.get('@product')
                metabolites.append({
                    "bigg_id": get_metabolite_name(metabolite_id, nodes, is_primary),
                    "coefficient": 1
                })
...
save_json_data(cytoscape_data, 'escher-v3.json')
